...
async def list_tasks(request: Request):
    tasks = []
    for doc in await request.app.mondodb["tasks"].find().to_list(length=100):
        tasks.append(doc)
...
async def show_task(id: str, reqest: Request):
    if (
        task := await reqest.app.mongodb["tasks"].find_one({"_id": id})
    ) is not None:
        return task
